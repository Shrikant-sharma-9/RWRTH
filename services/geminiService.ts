...
export const analyzeRTRWH = async (formData: FormData): Promise<AnalysisResult> => {
  const { location, dwellers, roofArea, openSpace } = formData;
  ...
  const prompt = `
    ...
    Analysis to perform:
    1.  **Feasibility Check**: Based on roof area, rainfall (infer for the location), and open space.
    2.  **Suggested Structure**: Recommend a suitable RTRWH or Artificial Recharge structure (e.g., Recharge Pit, Trench, Shaft, or simple Storage Tank) based on the available open space and likely soil conditions for the area.
